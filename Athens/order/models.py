...
class Order(models.Model):
    ...
    # last_name = models.CharField(max_length=50)
    # email = models.EmailField()
    # 고객과 외래키를 통해 관계를 맺음(1:N)
    customer =  models.ForeignKey(customer_tbl, on_delete=models.CASCADE, null=True)
    # 생성일
...
class OrderItem(models.Model):
    ...
    # 상품(강의)와 관계키를 맺음 (구매한 강의)
    # 강의 번호
    l_no = models.ForeignKey(lecture_tbl, on_delete=models.PROTECT, related_name='order_products', null=True)
...
class OrderTransaction(models.Model):
    ...
    # lecture_tbl(수강테이블)과 관계를 맺음.
    # lecture = models.ForeignKey(lecture_tbl, on_delete=models.CASCADE, null=True)
    # customer = models.ForeignKey(customer_tbl, on_delete=models.CASCADE, null=True)
    merchant_order_id = models.CharField(max_length=120, null=True, blank=True)
    transaction_id = models.CharField(max_length=120, null=True,blank=True)

...
def exam_enroll(request, l_no):
    ...
    # 테스트 등록일을 기준으로 내림차순 정렬. (최신 강의를 위쪽에에 나오 하기 위해)
    try:
        test_list = test_list.order_by('-te_join')
    except:
        test_list = []
...
def enrollment(request, l_no):
    ...
    if request.method == 'POST':
        if request.POST['btn'] == '1':
            ...
            # 문제 수를 받을 빈 리스트
            number_list = []
            # 문제 수(생성할 문제 수로 입력한 값) - 정수로 입력하지 않았을 시 예외처리
            try:
                number = int(request.POST['number'])
            except:
                return HttpResponse(
                    '<script type="text/javascript"> alert("문제수를 정수값으로 입력 해 주세요."); history.back();</script>')

            # 문제 수를 1~20 사이의 값을 입력하지 않았을 시 메세지를 띄웁니다.
            if not 0 < number < 21:
                return HttpResponse('<script type="text/javascript"> alert("문제수를 1~20 사이값으로 입력 해 주세요."); history.back();</script>' )
...
def exam_modify(request, l_no, te_no):
    ...
    if request.method == 'POST':
        if request.POST['btn'] == '2':
            ...
            for i in range(number):
                # 모두 입력했으면 pass
                if bool(mydict['question'][i]) and bool(mydict['choice_1'][i]) and bool(mydict['choice_2'][i]) and bool(
                        mydict['choice_3'][i]) and bool(mydict['choice_4'][i]) and bool(mydict['answer'][i]):
                    pass
                # 하나라도 입력하지 않았을 시
                else:
                    return HttpResponse('<script type="text/javascript"> alert("모든 값을 입력해 주세요."); history.back();</script>')
            ...
            print(request.POST)
            # 해당 test객체를 불러옵니다.
            test = test_tbl.objects.get(te_no = te_no)

            # 테스트명에 변경이 있다면 변경사항을 저장합니다.
            test.te_name = request.POST['te_name']
            test.save()
            ...
            for i in range(number):
                ...
                # 해당 테스트의 일련번호(외래키)와 문제 번호를 통해 문제 세부사항들을 불러옵니다.
                test_detail = test_detail_tbl.objects.get(td_question_no = mydict['question_no'][i], te_no_id=test.te_no)

                # 변경이 있으변 변경된 정보를 저장합니다.
                test_detail.td_question = mydict['question'][i]
                test_detail.td_choice_1 = mydict['choice_1'][i]
                test_detail.td_choice_2 = mydict['choice_2'][i]
                test_detail.td_choice_3 = mydict['choice_3'][i]
                test_detail.td_choice_4 = mydict['choice_4'][i]
                test_detail.td_answer = mydict['answer'][i]
                test_detail.save()
    ...
    # 필터를 통해 해당 시험 일련번호(te_no)에 맞는 test_detail 리스트를 가져온다.
    test_detail_list = test_detail_tbl.objects.filter(te_no_id=te_no)
    # 총 문제수 뽑아내기(리스트에 담긴 갯수가 총 문제수랑 일치한다.)
    number = len(test_detail_list)
    # 총 문제수를 리스트로 담기 위한 빈 리스트
    number_list = []
    for i in range(number):
        number_list.append(i+1)

    # 해당 시험의 객체를 가져 옵니다.(시험 명 가져오기 위해)
    test = test_tbl.objects.get(te_no = te_no)

    context = {'lecture_info': lecture_info, 'number_list': number_list, 'test_detail_list': test_detail_list, 'test' : test }

    return render(request, 'exam/enrollment_modify.html', context)

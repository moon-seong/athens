...
class LectureForm(forms.ModelForm):
    class Meta:
        model = lecture_tbl
        # l_subject 수정
        fields = ['l_name', 'l_totalnum', 'l_term', 'l_pay', 'l_startdate', 'l_desc', 'l_dept', 'l_class']

        widgets = {
            # 강의명
            'l_name': forms.TextInput(attrs={'class': 'form-control', 'rows': 5}),
            # 정원
            'l_totalnum': forms.TextInput(attrs={'class': 'form-control', 'placeholder' : '(명)','rows': 5}),
            'l_term': forms.Select(attrs={'class': 'form-control', 'rows': 5}),
            'l_pay': forms.TextInput(attrs={'class': 'form-control' ,'rows': 5, 'placeholder' : '(단위 : 원)'}),
            'l_startdate': forms.DateInput(attrs={'class': 'form-control', 'id' : 'date-text', 'rows': 5}),
...
class NoticedetailForm(forms.ModelForm):
    class Meta:
        model = notice_tbl
        fields = ['notice_no','notice_target','notice_title','notice_content']
